...
var restBuddy = module.exports = function (sequelize, options) {
  ...
  return function restBuddy (req, res, next) {
    var queryOptions = {};
...
var requestHandlers = Object.freeze({
  show: function show (model, queryOptions, options, req, res, next) {
    ...
    model.findOne(queryOptions)
      ...
      .catch(Sequelize.DatabaseError, handleDatabaseError(res))
      .catch(next);
  },
...
/**
 * Handle a thrown Sequelize.DatabaseError (or not).
 *
 * @param {Sequelize.DatabaseError} error
 * @param {object} response object
 */
function handleDatabaseError (res) {
  // Sequelize throws DatabaseError at all sorts of oppurtunities
  // We match on the error code to spot 'Data Exception' errors
  // See http://www.postgresql.org/docs/9.3/static/errcodes-appendix.html
  return function (err) {
    // Class 22 errors are data exceptions
    if (err.parent.code.match(/22[A-Z0-9]{3}/)) {
      return res.status(400).json({ message: err.message });
    // Class 23 errors represent integrity constraint violations
    } else if (err.parent.code.match(/23[A-Z0-9]{3}/)) {
      return res.status(400).json({ message: err.message });
    } else {
      throw err;
    }
